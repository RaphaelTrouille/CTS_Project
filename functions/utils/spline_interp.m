function [V2, L2, L1] = spline_interp(elc1, V1, elc2, order, degree, lambda)
...
%  end
% end
CosEii = 1-squeeze(sum(bsxfun(@minus, elc1, permute(elc1,[3 2 1])).^2,2)/2);
% compute matrix G of Perrin 1989
gx = gh(CosEii, order, degree);
...
%        end
%      end     
    CosEji = 1-squeeze(sum(bsxfun(@minus, elc2, permute(elc1,[3 2 1])).^2,2)/2);   
    gx = gh(CosEji, order, degree);
end

[dummy,B] = sort(size(gx));
gx    = permute(gx,B);
...
R = bsxfun(@rdivide,(2*(1:N)+1),(bsxfun(@times,1:N,(1:N)+1)).^M);
...
for i=1:size(x,1)
  ...
  for j=jloop
    ...
    % p = squeeze(P(i, j ,:))';
    gx(i,j) = sum(bsxfun(@times,R,p))/(4*pi);
